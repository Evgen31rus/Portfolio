...
export const  Works = forwardRef<HTMLDivElement>( (props, ref) => {
    ...
    const works = [
        ...
        },
        {
            titleRu: 'PRO AUTO',
            titleEn: 'PRO AUTO',
            discriptionRu: 'Интернет-магазин автоаксессуаров, аккумуляторов, моторных масел и другого, многостроничный сайт с формой обратной связи, глобальным состоянием в виде корзины, возможностью пополнять лист товаров, а так же отправкой заказа.',
...
viewport={{amount: 0.2}}
custom={index}
       index={index} titleRu={el.titleRu} titleEn={el.titleEn} discriptionEn={el.discriptionEn} discriptionRu={el.discriptionRu} urlCode={el.urlCode} urlPage={el.urlPage} img={el.img}/>
    )
}
        </div>
    )
